...
public class Server {
    ...
    public func verifySession(publicKey clientPublicKey: Data, keyProof clientKeyProof: Data) -> (Bool, Any) {
        ...
        guard A % N != 0 else {
            ...
            return (false, "Invalid client public key")
        }
        ...
        let M = calculate_M(group: group, algorithm: algorithm, u: u, salt: salt, A: clientPublicKey, B: publicKey, K: K!)
        ...
        return (true, calculate_HAMK(algorithm: algorithm, A: clientPublicKey, M: M, K: sessionKey!))
    }
    
    // This func is used only for SignUp. Not used for SignIn. Made by Alex Ngo
    public func calculateSessionKey(clientPublicKey: Data) -> (Bool, Any) {
        let u = calculate_u(group: group, algorithm: algorithm, A: clientPublicKey, B: publicKey)
        let A = BigUInt(clientPublicKey)
        let N = group.N
    ...
    /// The server's public key (B). For every authentication
    /// session a new public key is generated.
    public var publicKey: Data {
        return B.serialize()
    }

    /// The server's private key (b). For every authentication
