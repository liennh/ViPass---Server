...
extension RSA {
    ...
    static public func encrypt(data: Data, withKey key: Key, usingCipher cipher: Cipher, padding: Padding? = nil) throws -> Data {
        ...
        let sizeIVBuffer = EVP_CIPHER_iv_length(RSA.getCipher(cipher)).toInt()
        ...
        guard EVP_SealInit(context, RSA.getCipher(cipher), &pointerSimmetricKey, &lenghtSimmetricKey, pointerIVBuffer, &pointerAsimmetricKey, 1) != 0 else {
        ...
        let cipherBlockLenght = EVP_CIPHER_block_size(RSA.getCipher(cipher)).toInt()
    ...
    static public func decrypt(data: Data, withKey key: Key, usingCipher cipher: Cipher, padding: Padding? = nil) throws -> Data {
        ...
        guard let bundle = EncryptedBundle(fromData: data) else {
        ...
        guard EVP_OpenInit(context, RSA.getCipher(cipher), simmetricKeyPointer, bundle.encryptedSimmetricKey.count.toCInt(), ivPointer, key.key) != 0 else {
            throw Custom.error(description: OpenSSL.errorDescription)
        }
